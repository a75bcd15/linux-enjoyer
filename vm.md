...
# VirtualBox
## Объединить виртуальные машины в локальную сеть
1) В [VirtualBox](https://www.virtualbox.org/wiki/Downloads) перейти в Файл > Инструменты > Менеджер сетей;
2) Создать новую сеть;
3) Во вкладке DHCP сервер включить сервер;
4) В "Адаптере" поставить галочку "Настроить адаптер автоматически";
5) Настроить виртуальную машину. Во вкладке Сеть выбрать Адаптер 2 > Виртуальный адаптер хоста > в качестве имени выбрать новую сеть.

Командой `ifconfig`  можно проверить изменение адреса, а `ping x.x.x.x -c 5` связь между машинами. Интернета в такой сети нет.